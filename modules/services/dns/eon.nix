...
let
  cfg = config.eilean.services.dns;
in
lib.mkIf (cfg.enable && cfg.server == "eon") {
  services.eon = {
    enable = true;
    application = "capd";
    ...
    zoneFiles =
      ...
      in
      lib.attrsets.mapAttrsToList mapZonefile cfg.zones;
  };

  users.users = {
    eon.extraGroups = [ config.services.opendkim.group ];
  };

  ### bind prestart copy zonefiles
  systemd.services.eon.postStart =
    let
      update = ''
        update() {
          local file="$1"
      ...
      ops =
        let
          ...
          '';
        in
        lib.attrsets.mapAttrsToList mapZones cfg.zones;
    in
    update + builtins.concatStringsSep "\n" ops;
}
